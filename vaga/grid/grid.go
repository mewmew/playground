...
// Grid contains a grid. Each slot can hold a marker.
...
// Place places the marker at the specified grid slot.
func (g *Grid) Place(col, row int, mark Mark) (err error) {
	...
	if g[col][row] != MarkNone {
		return fmt.Errorf("Place: grid slot (%d, %d) not empty.", col, row)
	}
	g[col][row] = mark
	return nil
}
